import data1 from "./data/mobiles";
import './Cards.css';
...
export function MobileAccessories() {
  ...
  useEffect(() => {
    ...
    if (productList && productList.length > 0) {
      ...
      setFilteredProducts(productList); // Initialize filtered products as the full product list
    }
  }, []); // Empty array means the effect runs only once on mount

  // Handle search input and filter products based on search term
  ...
  return (
    <div>
      ...
      <Nav
        ...
        sticky="top">
        <Nav.Item>
          <Nav.Link href="/mobiles" style={{ color: "blue", fontSize: "16px", textDecoration:"underline" }}>Mobile & Wearable Tech</Nav.Link>
        </Nav.Item>
        <Nav.Item>
      ...
      <h1 style={{ color: "black", position: "absolute", left: "30%" }}>Product List</h1><br /><br /><br />
      <Container>
        <Row>
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product, index) => (
              <Col key={index} xs={12} sm={6} md={4} lg={3} className="mb-4">
                <Link to={`/mobiles/${product.id}`} className="card-link" style={{textDecoration:"none"}}>
                  <div className="card">
                    ...
                    <div className="card-body">
                      ...
                      <h3 className="card-price">Price: ${product.price}</h3>
