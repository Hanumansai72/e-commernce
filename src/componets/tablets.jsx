...
export default function Tablet() {
  ...
  const handleSearch = (e) => {
    const query = e.target.value.toLowerCase(); // Convert the search term to lowercase for case-insensitive search
  ...
  return (
    <div>
      ...
      <Nav activeKey="/home" sticky="top">
        ...
        <Nav.Item>
          <Nav.Link href="/tablet" style={{ color: "blue", fontSize: "16px", textDecoration: "underline" }}>Tablets</Nav.Link>
        </Nav.Item>
        <Nav.Item>
        ...
        <Nav.Item>
          <input 
            ...
            onChange={handleSearch} // Calls handleSearch on every keystroke
          ...
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product, index) => (
              <Col key={index} xs={12} sm={6} md={4} lg={3} className="mb-4">
                <Link to={`/tablet/${product.id}`} className="card-link" style={{ textDecoration: "none" }}>
                  <div className="card">
            ...
            <Col xs={12}><h2>No products available.</h2></Col>
          )}
        </Row>
      </Container>
      <Footer1 />
